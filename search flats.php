        ...
        <div class="row">
			...
			<?php
				if(isset($_POST['search']))
				{
					...
					$price		=$_POST['price'];
					$query="SELECT * FROM `flats` WHERE `status`='$status' OR `area`='$area' OR `location` LIKE '%$location%' OR `price`='$price' OR `rooms`='$rooms' OR `garage`='$garage'";
							...
							<div class="card-overlay-a-content">
							  <div class="card-header-a">
								<h2 class="card-title-a">
								  <a  href="property-single.php?p_id=<?php echo $data['id'];?>"> Land
